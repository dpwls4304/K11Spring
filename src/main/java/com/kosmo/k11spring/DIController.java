...
public class DIController {
	...
	public String myCal(Model model) {
		...
		/*
		 ...
		 ※webapp하위의 resource디렉토리는 이미지나 js파일 등
		...
		/*
		 ...
		 참조변수에 할당한다. new 연산자를통해 생성한것과 동일하지만
		 외부 생성파일에서 미리 생성한 것을 주입(Injection)받은것임에
		 유의할 것.
		 */
		CalculatorDTO myCal=
		...
		model.addAttribute("suvResult", myCal.sub());
	...
	//함수의 이름은 별 의미가 없음. 사용자가 구분하기 쉬운 이름으로.
	//체질량 지수 계산하기
	@RequestMapping("/di/myBMICal")
	public String BMICal(Model model) {
		
		//apllicationContext파일 생성 및 경로설정
	...
	public ModelAndView myAvengers() {
		...
		/*
		 ...
		 아래 classpath 키워드는 Bulid Path메뉴에서 확인 가능함.
		 */
